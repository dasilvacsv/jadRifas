...
import { notifyNewPurchase } from '@/websocket-server';
 ...
 * @param whatsapp Número de WhatsApp (opcional)
 */
async function createResendContact(email: string, name: string, whatsapp?: string): Promise<void> {
    const AUDIENCE_ID = process.env.RESEND_AUDIENCE_ID;

    if (!AUDIENCE_ID) {
        console.warn("ADVERTENCIA: La variable RESEND_AUDIENCE_ID no está configurada. No se creará el contacto en Resend.");
        return;
    }
    ...
    try {
        ...
        if (error) {
            console.error("Error al crear el contacto en Resend:", error);
            // No lanzamos error para no bloquear la transacción principal
        }
        console.log(`✅ Contacto en Resend creado/actualizado con éxito para: ${email}`);
    } catch (error) {
        ...
        if (purchaseStatus === 'confirmed') {
            ...
            try {
                ...
                if (purchaseWithRaffle) {
                    // Llama a la función que notifica a todos los clientes conectados
                    notifyNewPurchase(purchaseWithRaffle);
                }
            } catch (wsError) {
                console.error("Error al notificar vía WebSocket:", wsError);
            }
